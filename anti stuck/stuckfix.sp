...
public Action Timer_CheckPosition(Handle timer, any data)
{
	...
	if( client && IsClientInGame(client) && IsPlayerAlive(client) )
	{
		if( IsNotMoving(client) )
		{
			...
			{
				/*
				Sends an input to tell all existing nextbots to Assault (stop hiding)
				Seems to fix SI not climbing ladders if they were wanting to evade or something which could be the reason for SI like boomers & spitters not climbing.
				Bugs: Smokers wont retreat when they've smoked and this is already fired.
				*/
			...
			{
				L4D2_RunScript("GetPlayerFromUserID(%i).TryGetPathableLocationWithin(%i)", client, 500);
				TryToPush(client);
			}
		}
		else {
