...
void CreateCameraEntity( float position[3], float angles[3], bool bSave = false )
{
	...
	if( IsValidEdict(iEntity))
    {
		position[2] += 25.0;	// Get the camera of ground a bit increase z pos
		...
		if( bSave )
		{
			...
			// Need to disable afterwards or spectators risk getting their view stuck in random objects
			// Takes 15 seconds between when survival clock stops and round actually ends,
			// 16 seconds between when survival clock stops and round_start is called. if timer >= 16.0 entity won't get disabled.
			CreateTimer(15.8, Timer_KillViewcontroller, iEntity);
		}
    }
}

public Action Timer_KillViewcontroller(Handle timer, any data)
{
	int iEntity = data;
	if( IsValidEdict(iEntity))
	{
		...
		if (StrEqual(sEntityName, "point_viewcontrol_multiplayer")) // just to be safe.. 
		{
			AcceptEntityInput(iEntity, "disable"); // "kill" doesn't work
		}
	}
}
